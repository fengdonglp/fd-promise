...
;(function (factory, global) {
  ...
  function Promise (resolver) {
    ...
    this._resolveCallback = noop
    this._rejectCallback = noop
    this._id = ID++
  ...
  Promise.prototype = {
    ...
    then: function (onFulfilled, onRejected) {
      ...
      switch (this._status) {
        case PENDING:
          this._resolveCallback = makeCallback(promise, onFulfilled, 'resolve')
          this._rejectCallback = makeCallback(promise, onRejected, 'reject')
          break
        case FULFILLED:
   ...
   */
  function solveProcess (promise, x, action) {
    if (x === promise) {
   ...
   * @param {Any} x onFulfilled/onRejected 返回值
   */
  function tryThenable (promise, x, action) {
    // x为对象或函数
